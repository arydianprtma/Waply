                  ...
                  {(!isAddonMode && currentPlan.price === 0) ? (
                    ...
                    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                      {(() => {
                        ...
                        const activeBadges = enabledChannels
                          .filter((c) => c !== "snap" && channelMap[c])
                          .map((c) => channelMap[c]);
                        const displayedBadges = activeBadges.length > 0 ? activeBadges : ["BRI (BRIVA)"];
                        ...
                        return (
                          <>
                              ...
                              <div className="space-y-1">
                                ...
                                <p className="text-[11px] sm:text-xs text-slate-600 leading-relaxed">
                                  {displayedBadges.length === 1 ? (
                                    <>
                                      Metode pembayaran <strong>{displayedBadges[0]}</strong> akan langsung tampil di jendela aman <strong>Midtrans Popup</strong> saat Anda menekan tombol <strong>Bayar Sekarang</strong>.
                                    </>
                                  ) : (
                                    <>
                                      Pilihan metode pembayaran ({displayedBadges.slice(0, 3).join(", ")}{displayedBadges.length > 3 ? ", dll." : ""}) akan langsung tampil di jendela aman <strong>Midtrans Popup</strong> saat Anda menekan tombol <strong>Bayar Sekarang</strong>.
                                    </>
                                  )}
                                </p>
                              </div>
                            ...
                            <div className="pt-2 border-t border-slate-200/80 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-700 font-semibold">
                              ...
                              {displayedBadges.map((badgeName) => (
                                <span
                                  key={badgeName}
                                  className="px-2.5 py-0.5 bg-white border border-slate-200 rounded-lg shadow-2xs text-slate-800 font-bold flex items-center gap-1.5"
                                >
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between text-slate-600">
                      ...
                      <span className="font-bold text-emerald-600">
                        {enabledChannels.length === 1 && enabledChannels.includes("bri_va")
                          ? "Midtrans (BRI / BRIVA)"
                          : "Midtrans Otomatis (QRIS / VA)"}
                      </span>
                    </div>
